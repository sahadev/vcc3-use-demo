...
function useLockScroll(rootRef, shouldLock) {
  ...
  var onTouchMove = (event) => {
    ...
    var el = use.getScrollParent(event.target, rootRef.value);
    var {
      scrollHeight,
      offsetHeight,
      scrollTop
...
var CalendarMonth = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var [visible, setVisible] = use.useToggle();
    var daysRef = vue.ref();
    ...
    var scrollToDate = (body, targetDate) => {
      if (daysRef.value) {
        var daysRect = use.useRect(daysRef.value);
...
var _Sticky = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var root = vue.ref();
    var scrollParent = use.useScrollParent(root);
    var state = vue.reactive({
      fixed: false,
      width: 0,
      height: 0,
    ...
    var offset = vue.computed(() => unitToPx(props.position === "top" ? props.offsetTop : props.offsetBottom));
    ...
    var onScroll = () => {
      ...
      var rootRect = use.useRect(root);
      var scrollTop = getScrollTop(window);
      state.width = rootRect.width;
      state.height = rootRect.height;
      if (position === "top") {
        if (container) {
          var containerRect = use.useRect(container);
        ...
        if (container) {
          var _containerRect = use.useRect(container);
...
var _Tabs = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var wrapRef = vue.ref();
    var id = useId();
    var scroller = use.useScrollParent(root);
    ...
    var getCurrentIndexOnScroll = () => {
      for (var index2 = 0; index2 < children.length; index2++) {
        var {
          top
        } = use.useRect(children[index2].$el);
...
var _CollapseItem = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    vue.watch(expanded, (value, oldValue) => {
      ...
      var tick = value ? vue.nextTick : use.raf;
      tick(() => {
...
var _CouponList = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var updateListHeight = () => {
      var TABS_HEIGHT = 44;
      var rootHeight = use.useRect(root).height;
      var headerHeight = use.useRect(barRef).height + TABS_HEIGHT;
...
var renderStop = (color, offset, opacity) => vue.createVNode("stop", {
  "stop-color": color,
  "offset": offset + "%",
...
var VanImagePreview = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var resize = () => {
      if (swipeRef.value) {
        var rect = use.useRect(swipeRef.value.$el);
...
var _IndexBar = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var onScroll = () => {
      ...
      var scrollParentRect = use.useRect(scrollParent);
...
var getDPR = function(scale) {
  ...
  return use.inBrowser ? window.devicePixelRatio || scale : scale;
};
  ...
  setup(props, _ref) {
    ...
    var check = () => {
      vue.nextTick(() => {
        ...
        var placeholderRect = use.useRect(placeholder);
        if (direction === "up") {
  ...
  setup(props, _ref) {
    ...
    var reset = () => {
      ...
      startTimer = setTimeout(() => {
        ...
        var wrapRefWidth = use.useRect(wrapRef).width;
        var contentRefWidth = use.useRect(contentRef).width;
...
var _Rate = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var ranges;
    var updateRanges = () => {
      var rects = itemRefs.value.map(use.useRect);
      ranges = [];
...
var _SwipeCell = vue.defineComponent({
  ...
  setup(props, _ref) {
    ...
    var touch = useTouch();
    var getWidthByRef = (ref) => ref.value ? use.useRect(ref).width : 0;
